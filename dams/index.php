...
if($appdate<=$cdate){
 ...
 Appointment date must be greater than todays date
</div>';
} else {
...
<div class="col-lg-6 col-12">
        <select onChange="getdoctors(this.value);"  name="specialization" id="specialization" class="form-control" required>
            ...
            <?php
            $sql = "SELECT ID, Specialization FROM tblspecialization";
            $stmt = $dbh->query($sql);
            $stmt->setFetchMode(PDO::FETCH_ASSOC);
            while ($row = $stmt->fetch()) { 
                echo '<option value="' . htmlspecialchars($row['ID']) . '">' . htmlspecialchars($row['Specialization']) . '</option>';
            }
            ?>
</select>
        </div>
<!--- Fetching States--->
    ...
    <div class="col-lg-6 col-12">
        ...
        <select name="doctorlist" id="doctorlist" class="form-control">
            ...
            <?php
    // Fetch doctors dynamically when specialization is selected
    if (!empty($_POST["specialization_id"])) {
        $specialization_id = $_POST["specialization_id"];

        try {
            // Fetch doctors based on specialization
            $sql = "SELECT ID, FullName FROM tbldoctor WHERE Specialization = :specialization_id";
            $stmt = $dbh->prepare($sql);
            $stmt->execute([':specialization_id' => $specialization_id]);
            $doctors = $stmt->fetchAll(PDO::FETCH_ASSOC);
            // Generate and return <option> elements
            foreach ($doctors as $doctor) {
                echo '<option value="' . htmlspecialchars($doctor['ID']) . '">' . htmlspecialchars($doctor['FullName']) . '</option>';
            }
        ...
        $(document).ready(function() {
            $('#specialization').on('change', function() {
                ...
                if (specializationId) {
                    $.ajax({
                        type: 'POST',
                        url: '', // This same PHP file
                        data: { specialization_id: specializationId },
                        success: function(response) {
                            $('#doctorlist').html('<option value="">Select Doctor</option>' + response);
                        },
                        error: function() {
